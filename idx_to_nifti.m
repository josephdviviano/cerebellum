function idx_to_nifti(file, idx, rois, outname)
	% Takes an input mask, a set of new labels (one for each unique value in
    ...
    for roi = rois';
        ...
        %disp([num2str(roi) ' = ' num2str(idx(count))]);
        count = count+1;
    end

    output.img = reshape(output.img, [dims(1), dims(2), dims(3)]);
    output.hdr.dime.datatype = 64;
