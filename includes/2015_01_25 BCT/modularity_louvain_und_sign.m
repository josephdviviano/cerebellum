...
%   Joseph D Viviano, CAMH, 2015

%   Modification History:
%   Mar 2011: Original
%   Apr 2015: Added fix for infinite loop cases
...
if ~s0; s0=1; d0=0; end % adjust for absent positive weights
if ~s1; s1=1; d1=0; end % adjust for absent negative weights
...
while Q{h}-Q{h-1} > 1e-10;
    ...
    count = 0; % keeps track of attempts
    maxiter = 100000; % set max attempts
    while f, f=0;
        for u = randperm(n); % loop over all nodes in random order
            ...
            % if we trip maxiter, set f to 0, even if there was a  
            if count > maxiter; % positive increase in modularity.
                f = 0;
            end
            count = count + 1;
